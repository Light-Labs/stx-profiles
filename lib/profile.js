...
export async function fetchProfilesByName({ name, address }) {
  ...
  if (data) {
    console.log(data);
    const img = getImageFromProfile(data);
    publicProfile = {
      nameOwner: address,
      data: {
...
function getImageFromProfile(data) {
  return (
    (typeof data?.image === "string"
      ? data.image
      : data?.image[0]?.contentUrl) || DEFAULT_IMAGE
  );
}
/**
 ...
 */
export function createImageForProfile(profile, imageUrl, caip19Id) {
  const imageObject = avatarImageObject(imageUrl, caip19Id);
  if (!profile) {
    return [imageObject];
  }
  ...
  if (imageType === "array") {
    ...
    imageList[0] = imageObject;
    return imageList;
  } else {
    // default to an array with on image only
    return [imageObject];
  }
}

function avatarImageObject(imageUrl, caip19Id) {
  return {
    "@type": "ImageObject",
    name: "avatar",
    contentUrl: imageUrl,
...
export function createCAIP20ID({
  contractAddress,
  contractName,
  assetName,
  id,
}) {
  return `stacks:1/sip9/${contractAddress}.${contractName}/${id}`;
}
